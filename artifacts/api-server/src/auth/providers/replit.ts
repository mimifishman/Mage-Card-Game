...
export class ReplitAuthService implements AuthService {
  ...
  async handleWebLogin(req: Request, res: Response): Promise<void> {
    const config = await getOidcConfig();
  ...
  async handleWebCallback(req: Request, res: Response): Promise<void> {
    const config = await getOidcConfig();
    const callbackUrl = `${getOrigin(req)}/api/callback`;
  ...
  async handleWebLogout(req: Request, res: Response): Promise<void> {
    const config = await getOidcConfig();
